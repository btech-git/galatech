...
class PurchaseReceiptController extends Controller {
    ...
    public function actionCreate() {
        ...
        $purchaseInvoiceDataProvider = $purchaseInvoice->searchByPurchaseReceipt();
        $purchaseInvoiceDataProvider->criteria->with = array('purchaseHeader');

        if (!empty($supplierId)) {
            $purchaseInvoiceDataProvider->criteria->addCondition("purchaseHeader.supplier_id = :supplier_id");
            $purchaseInvoiceDataProvider->criteria->params[':supplier_id'] = $supplierId;
        }
        
        if (empty($purchaseReceipt->header->id)) {
...
//            $purchaseReceipt->header->number = CodeNumber::make($purchaseReceipt->header, 'number', ($purchaseReceipt->header->is_non_tax) ? 'NTTR' : 'TTR', ($purchaseReceipt->header->is_non_tax) ? true : false);
        ...
        if (isset($_POST['Submit']) && IdempotentManager::check()) {
            ...
            try {
                if ($purchaseReceipt->validate() && IdempotentManager::build()->save() && $purchaseReceipt->save(false)) {

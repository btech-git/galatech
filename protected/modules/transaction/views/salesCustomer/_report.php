...
<table class="report">
        ...
        <?php foreach ($dataProvider->data as $header): ?>
                ...
                <tr class="items2">
                        <td colspan="2">
                                <table>
                                        <?php foreach ($header->deliveryHeaders as $detail): ?>
                                                <tr>
                                                        <td class="width2-1"><?php echo CHtml::encode(CHtml::value($detail, 'number')); ?></td>
                                                        <td class="width2-2"><?php echo CHtml::encode(Yii::app()->dateFormatter->format('dd MMMM yyyy', strtotime($detail->date))); ?></td>
										...
										<tr>
											...
											<td class="width2-3" style="border-top: 1px solid; font-weight: bold; text-align: right"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', ceil(CHtml::value($header, 'totalSales')))); ?></td>
										</tr>
                                </table>
                        </td>
                </tr>
        <?php endforeach; ?>
		<tr>
                <td class="width2-1" style="border-top: 1px solid; font-weight: bold; text-align: right">TOTAL PENJUALAN</td>
                <td class="width2-2" style="border-top: 1px solid; font-weight: bold; text-align: right"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', ceil($this->reportGrandTotal($dataProvider)))); ?></td>
        </tr>
</table>

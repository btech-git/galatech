<?php if ($error === true && count($salesCheque->details) === 0): ?>
...
<table style="border: 1px solid">
	...
	<?php foreach ($salesCheque->details as $i=>$detail): ?>
		<tr style="background-color: azure">
			...
			<td style="text-align: right">
				<?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', CHtml::value($detail, 'receiptHeader.totalInvoice'))); ?>
			</td>
			...
			<td style="text-align: center">
				<?php echo CHtml::activeTextField($detail, "[$i]amount", array(
					'onchange'=>CHtml::ajax(array(
						'type'=>'POST',
						'dataType'=>'JSON',
						'url'=>CController::createUrl('summaryAjaxData', array('id'=>$salesCheque->header->id, 'index'=>$i)),
			...
			<td style="text-align: center">
				<?php if (TaxConnectionChecking::isCurrentConnectionSecondary()): ?>
					<?php echo CHtml::activeDropDownList($detail, "[$i]account_id", CHtml::listData(Account::model()->findAll($salesChequeAccountCategorySecondary), 'id', 'name'), array('empty' => '--Pilih Bank--')); ?>
				 <?php else: ?>
	...
	<tr style="background-color: aquamarine">
		...
		<td style="font-weight: bold; text-align: right"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $salesCheque->totalReceipt)); ?></td>
		<td style="font-weight: bold">TOTAL</td>
		<td style="font-weight: bold; text-align: right">
			<span id="total">
				<?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $salesCheque->total)); ?>
			</span>
		</td>
		<td colspan="3"></td>
	</tr>

<?php if ($error === true && count($purchaseAsset->details) === 0): ?>
...
<table style="border: 1px solid">
        ...
        <?php foreach ($purchaseAsset->details as $i=>$detail): ?>
                ...
                <td style="text-align: center; width: 10%">
                        <?php echo CHtml::activeTextField($detail, "[$i]quantity", array('size'=>7, 'maxLength'=>20,
                                'onchange'=>CHtml::ajax(array(
                                        'type'=>'POST',
                                        'dataType'=>'JSON',
                                        'url'=>CController::createUrl('totalAjaxData', array('id'=>$purchaseAsset->header->id, 'index'=>$i)),
                        ...
                        <?php if ($detail->isNewRecord): ?>
                                <?php echo CHtml::button('Delete', array(
                                        'onclick'=>CHtml::ajax(array(
                                                'type'=>'POST',
                                                'url'=>CController::createUrl('removeProductAjax', array('id'=>$purchaseAsset->header->id, 'index'=>$i)),
                                                'update'=>'#detail_div',
                                        )),
                                )); ?>
                        <?php else: ?>
        ...
        <tr style="background-color: aquamarine">
                ...
                <td style="text-align: right; font-weight: bold">
                        <span id="sub_total">
                                <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $purchaseAsset->subTotal)); ?>
                        </span>
                </td>
                <td></td>
        </tr>

<?php if ($error === true && count($purchase->details) === 0): ?>
    <p style="color: red">Form tidak ada data untuk insert database. Minimal satu data detail untuk melakukan penyimpanan.</p>
<?php endif; ?>

<table style="border: 1px solid">
    <tr style="background-color: skyblue">
        <th style="text-align: center">Nama Barang</th>
        <th style="text-align: center">Ukuran</th>
        <th style="text-align: center">Jumlah</th>
        <th style="text-align: center">Satuan</th>
        <th style="text-align: center">Harga Satuan</th>
        <th style="text-align: center">Harga + PPn</th>
        <!--<th style="text-align: center">Diskon (%)</th>-->
        <th style="text-align: center">Total</th>
        <th style="text-align: center"></th>
    </tr>
    <?php foreach ($purchase->details as $i => $detail): ?>
        <?php //if ($detail->is_inactive == 0): ?>
            <tr style="background-color: azure">
                <td style="width: auto">
                    <?php echo CHtml::activeHiddenField($detail, "[$i]product_id"); ?>
                    <?php echo CHtml::encode(CHtml::value($detail, 'product.name')); ?>
                </td>
                <td style="text-align: center; width: 5%">
                    <?php echo CHtml::encode(CHtml::value($detail, 'product.size')); ?>
                </td>
                <td style="text-align: center; width: 10%">
                    <?php echo CHtml::activeTextField($detail, "[$i]quantity", array(
                        'size' => 7, 'maxLength' => 20,
                        'onchange' => CHtml::ajax(array(
                            'type' => 'POST',
                            'dataType' => 'JSON',
                            'url' => CController::createUrl('totalAjaxData', array('id' => $purchase->header->id, 'index' => $i)),
                            'success' => 'function(data) {
                                $("#total_' . $i . '").html(data.total);
                                $("#price_before_tax_' . $i . '").html(data.priceBeforeTaxFormat);
                                $("#' . CHtml::activeId($detail, "[$i]price_before_tax") . '").val(data.priceBeforeTax);
                                $("#sub_total").html(data.subTotal);
                                $("#sub_total_quantity").html(data.subTotalQuantity);
                                $("#discount").html(data.discount);
                                $("#tax").html(data.tax);
                                $("#grand_total").html(data.grandTotal);
                            }',
                        )),
                    ));
                    ?>
                    <?php echo CHtml::error($detail, 'quantity'); ?>
                </td>
                <td style="text-align: center; width: 5%">
                    <?php echo CHtml::encode(CHtml::value($detail, 'product.unit.name')); ?>
                    <?php echo CHtml::error($detail, 'product.unit.name'); ?>
                </td>
                <td style="text-align: right; width: 15%">
                    <?php echo CHtml::activeTextField($detail, "[$i]unit_price", array(
                        'size' => 10, 'maxLength' => 20,
                        'onchange' => CHtml::ajax(array(
                            'type' => 'POST',
                            'dataType' => 'JSON',
                            'url' => CController::createUrl('totalAjaxData', array('id' => $purchase->header->id, 'index' => $i)),
                            'success' => 'function(data) {
                                $("#unit_price_' . $i . '").html(data.unitPrice);
                                $("#price_before_tax_' . $i . '").html(data.priceBeforeTaxFormat);
                                $("#' . CHtml::activeId($detail, "[$i]price_before_tax") . '").val(data.priceBeforeTax);
                                $("#total_' . $i . '").html(data.total);
                                $("#sub_total").html(data.subTotal);
                                $("#sub_total_quantity").html(data.subTotalQuantity);
                                $("#discount").html(data.discount);
                                $("#tax").html(data.tax);
                                $("#grand_total").html(data.grandTotal);
                            }',
                        )),
                    )); ?>
                    <div id="unit_price_<?php echo $i; ?>" style="text-align: right; font-size: smaller">
                        <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', CHtml::value($detail, 'unit_price'))); ?>
                    </div>
                    <?php echo CHtml::error($detail, 'unit_price'); ?>
                </td>
                <td style="text-align: right">
                    <?php echo CHtml::activeHiddenField($detail, "[$i]price_before_tax"); ?>
                    <span id="price_before_tax_<?php echo $i; ?>">
                        <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', CHtml::value($detail, 'price_before_tax'))); ?>
                    </span>
                </td>
    <!--            <td style="text-align: center; width: 10%">
                    <?php /*echo CHtml::activeTextField($detail, "[$i]discount", array(
                        'size' => 3, 'maxLength' => 10,
                        'onchange' => CHtml::ajax(array(
                            'type' => 'POST',
                            'dataType' => 'JSON',
                            'url' => CController::createUrl('totalAjaxData', array('id' => $purchase->header->id, 'index' => $i)),
                            'success' => 'function(data) {
                                $("#total_' . $i . '").html(data.total);
                                $("#price_before_tax_' . $i . '").html(data.priceBeforeTaxFormat);
                                $("#' . CHtml::activeId($detail, "[$i]price_before_tax") . '").val(data.priceBeforeTax);
                                $("#sub_total").html(data.subTotal);
                                $("#sub_total_quantity").html(data.subTotalQuantity);
                                $("#discount").html(data.discount);
                                $("#tax").html(data.tax);
                                $("#grand_total").html(data.grandTotal);
                            }',
                        )),
                    )); ?>
                    <?php echo CHtml::error($detail, 'discount');*/ ?>
                </td>-->
                <td style="text-align: right; width: 15%">
                    <span id="total_<?php echo $i; ?>">
                        <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $detail->getTotal($purchase->header->tax_type, $purchase->header->tax))); ?>
                    </span>
                </td>
                <td style="width: 5%">
                    <?php if ($detail->isNewRecord): ?>
                        <?php echo CHtml::button('Delete', array(
                            'onclick' => CHtml::ajax(array(
                                'type' => 'POST',
                                'url' => CController::createUrl('removeProductAjax', array('id' => $purchase->header->id, 'index' => $i)),
                                'update' => '#detail_div',
                            )),
                        )); ?>
                    <?php else: ?>
                        <?php echo CHtml::activeDropDownList($detail, "[$i]is_inactive", array(
                            ActiveRecord::ACTIVE => 'Active', 
                            ActiveRecord::INACTIVE => 'Inactive'
                        )); ?>
                    <?php endif; ?>
                </td>
            </tr>
        <?php //endif; ?>
    <?php endforeach; ?>
    <tr style="background-color: aquamarine">
        <td style="text-align: right" colspan="2">Total Qty</td>
        <td style="text-align: right">
            <span id="sub_total_quantity">
                <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', $purchase->subTotalQuantity)); ?>
            </span>
        </td>
        <td style="text-align: right" colspan="3">Sub Total:</td>
        <td style="text-align: right">
            <span id="sub_total">
                <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $purchase->subTotal)); ?>
            </span>
        </td>
        <td></td>
    </tr>
    <tr style="background-color: aquamarine">
        <td style="text-align: right" colspan="6">
            <?php echo CHtml::activeLabelEx($purchase->header, 'discount', array('style' => 'display: inline')); ?>
            <?php echo CHtml::activeTextField($purchase->header, 'discount', array(
                'size' => 2, 'maxlength' => 2,
                'onchange' => CHtml::ajax(array(
                    'type' => 'POST',
                    'dataType' => 'JSON',
                    'url' => CController::createUrl('grandTotalAjaxData', array('id' => $purchase->header->id)),
                    'success' => 'function(data) {
                        $("#discount").html(data.discount);
                        $("#tax").html(data.tax);
                        $("#grand_total").html(data.grandTotal);
                    }',
                )),
            )); ?>
            <?php echo '%'; ?>
        </td>
        <td style="text-align: right">
            <span id="discount">
                <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $purchase->calculatedDiscount)); ?>
            </span>
        </td>
        <td></td>
    </tr>
    <tr style="background-color: aquamarine">
        <td style="text-align: right" colspan="6">
            PPn <?php echo CHtml::activeDropDownList($purchase->header, 'tax_type', array(
                PurchaseHeader::TAX_TYPE_INCLUDE => PurchaseHeader::TAX_TYPE_INCLUDE_LITERAL,
                PurchaseHeader::TAX_TYPE_EXCLUDE => PurchaseHeader::TAX_TYPE_EXCLUDE_LITERAL,
            ), array(
                'onchange' => CHtml::ajax(array(
                    'type' => 'POST',
                    'dataType' => 'JSON',
                    'url' => CController::createUrl('grandTotalAjaxData', array('id' => $purchase->header->id)),
                    'success' => 'function(data) {
                        $("#tax").html(data.tax);
                        $("#grand_total").html(data.grandTotal);
                    }',
                )) . CHtml::ajax(array(
                    'type' => 'POST',
                    'url' => CController::createUrl('updateAllPriceAjax'),
                    'update' => '#detail_div',
                )),
            )); ?> 
            <?php echo CHtml::activeTextField($purchase->header, 'tax', array(
                'size' => 2, 'maxlength' => 2,
                'onchange' => CHtml::ajax(array(
                    'type' => 'POST',
                    'dataType' => 'JSON',
                    'url' => CController::createUrl('grandTotalAjaxData', array('id' => $purchase->header->id)),
                    'success' => 'function(data) {
                        $("#tax").html(data.tax);
                        $("#grand_total").html(data.grandTotal);
                    }',
                )) . CHtml::ajax(array(
                    'type' => 'POST',
                    'url' => CController::createUrl('updateAllPriceAjax'),
                    'update' => '#detail_div',
                )),
            )); ?>
            <?php echo '%'; ?>
        </td>
        <td style="text-align: right">
            <span id="tax">
                <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $purchase->calculatedTax)); ?>
            </span>
        </td>
        <td></td>
    </tr>
    <?php if ($purchase->header->is_non_tax): ?>
        <tr style="background-color: aquamarine">
            <td style="text-align: right" colspan="6">
                <?php echo CHtml::activeLabelEx($purchase->header, 'shipping_fee'); ?>
            </td>
            <td style="text-align: right">
                <?php echo CHtml::activeTextField($purchase->header, 'shipping_fee', array(
                    'onchange' => CHtml::ajax(array(
                        'type' => 'POST',
                        'dataType' => 'JSON',
                        'url' => CController::createUrl('grandTotalAjaxData', array('id' => $purchase->header->id)),
                        'success' => 'function(data) {
                            $("#grand_total").html(data.grandTotal);
                        }',
                    )),
                )); ?>
            </td>
            <td></td>
        </tr>
    <?php endif; ?>
    <tr style="background-color: aquamarine">
        <td style="text-align: right" colspan="6">Grand Total:</td>
        <td style="text-align: right">
            <span id="grand_total">
                <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $purchase->grandTotal)); ?>
            </span>
        </td>
        <td></td>
    </tr>
</table>
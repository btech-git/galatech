...
class Deposit extends CComponent {
    ...
    public function __construct() {
        $this->header = new DepositHeader();
        $this->details = array();
    }

    public function copyFromDb($id) {
        $this->header = DepositHeader::model()->resetScope()->findByPk($id);
        $this->details = DepositDetail::model()->resetScope()->findAllByAttributes(array('deposit_header_id' => $id));
    }

	...
	<tr id="header1">
        ...
        <?php foreach ($dataProvider->data as $header): ?>
			<tr class="items1">
				...
				<td class="width1-2"><?php echo CHtml::encode(Yii::app()->dateFormatter->format('d MMMM yyyy', strtotime($header->receive_date))); ?></td>
			...
			<tr class="items2">
				<td colspan="4">
					<table>
						<?php foreach ($header->salesChequeDetails as $detail): ?>
							<tr>
								<td class="width2-1"><?php echo CHtml::encode(CHtml::value($detail, 'receiptHeader.number')); ?></td>
								<td class="width2-2" style="text-align: right"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', CHtml::value($detail, 'receiptHeader.totalInvoice'))); ?></td>

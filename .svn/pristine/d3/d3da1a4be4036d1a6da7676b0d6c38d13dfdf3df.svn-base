	...
	<tr id="header2">
		<td colspan="6">
			<table>
				<tr>
					...
					<th class="width2-3">Jml Beli</th>
					<th class="width2-4">Jml Retur</th>
        ...
        <?php foreach ($dataProvider->data as $header): ?>
			<tr class="items1">
				...
				<td class="width1-3"><?php echo CHtml::encode(CHtml::value($header, 'purchaseInvoice.number')); ?></td>
				<td class="width1-4" style="text-align: right"><?php echo CHtml::encode(CHtml::value($header, ($header->is_non_tax) ? 'purchaseInvoice.purchaseHeader.supplier.name' : 'purchaseInvoice.purchaseHeader.supplier.company')); ?></td>
<!--                        <td class="width1-5" style="text-align: right"><?php //echo CHtml::encode(CHtml::value($header, 'receiveHeader.warehouse.name')); ?></td>-->
        ...
        <tr>
			...
			<td class="width1-5" style="border-top: 1px solid; font-weight: bold; text-align: right"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', ceil($this->reportGrandTotal($dataProvider)))); ?></td>
        </tr>
</table>
...
class SaleProductFifoPriceController extends SelectionController {
...
//            'access',
    ...
    public function actionReport() {
        ...
        $saleProductFifoPriceReport = DeliveryDetail::getSaleProductFifoPriceReport($startDate, $endDate, $customerName, $productName, $productSize);
        
        $deliveryDetailIds = array_map(array($this, 'getDeliveryDetailId'), $saleProductFifoPriceReport);
        $deliveryDetailIdInSql = empty($deliveryDetailIds) ? 'NULL' : implode(',', $deliveryDetailIds);
        $deliveryPurchaseCartInfoList = DeliveryPurchaseCartInfo::model()->findAll(array(
            'condition' => "delivery_detail_id IN ({$deliveryDetailIdInSql})", 
            'order' => 't.id ASC',
        ));
        $quantityAndUnitPriceInfo = array();
        foreach ($deliveryPurchaseCartInfoList as $deliveryPurchaseCartInfo) {
            if (!isset($quantityAndUnitPriceInfo[$deliveryPurchaseCartInfo->delivery_detail_id])) {
                $quantityAndUnitPriceInfo[$deliveryPurchaseCartInfo->delivery_detail_id] = array();
            }
            $quantityAndUnitPriceInfo[$deliveryPurchaseCartInfo->delivery_detail_id][] = array(
                'quantity' => $deliveryPurchaseCartInfo->quantity, 
                'unit_price' => $deliveryPurchaseCartInfo->unit_price
            );
        }
        
        if (isset($_GET['SaveExcel'])) {
            $this->saveToExcel($saleProductFifoPriceReport, array('startDate' => $startDate, 'endDate' => $endDate));
        }

        $this->render('report', array(
            'saleProductFifoPriceReport' => $saleProductFifoPriceReport,
            'startDate' => $startDate,
            'endDate' => $endDate,
            'customerName' => $customerName,
            'productName' => $productName, 
            'productSize' => $productSize,
            'quantityAndUnitPriceInfo' => $quantityAndUnitPriceInfo,
        ));
    }
    
    public function getDeliveryDetailId($saleProductFifoPriceReportItem) {
        return $saleProductFifoPriceReportItem['id'];
    }
    
    public function getHppInfo($infoItem) {
        return $infoItem['quantity'] . ': ' . $infoItem['unit_price'];
    }
    
    protected function saveToExcel($saleProductFifoPriceReport, array $options = array()) {
        set_time_limit(0);
        ...
        $worksheet->setCellValue('A2', 'Laporan Profit Penjualan Barang (FIFO)');
        ...
        $counter = 7;
        foreach ($saleProductFifoPriceReport as $i => $dataItem) {
...
//            if (isset($quantityAndUnitPriceInfo[$dataItem['id']])) {
//            $worksheet->setCellValue("I{$counter}", $dataItem['average_purchase_price']);
        ...
        header('Content-Disposition: attachment;filename="profit_penjualan_fifo.xls"');

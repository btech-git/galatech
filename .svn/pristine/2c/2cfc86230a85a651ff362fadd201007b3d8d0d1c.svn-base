...
class PurchaseAsset extends CComponent {

    public $header;
    public $details;

    public function __construct() {
        $this->header = new PurchaseAssetHeader();
        $this->details = array();
    }

    public function copyFromDb($id) {
        $this->header = PurchaseAssetHeader::model()->resetScope()->findByPk($id);
        $this->details = PurchaseAssetDetail::model()->resetScope()->findAllByAttributes(array('purchase_asset_header_id' => $id));
    }
    ...
    public function save($runValidation) {
        ...
        foreach ($this->details as $detail) {
            $detail->purchase_asset_header_id = $this->header->id;

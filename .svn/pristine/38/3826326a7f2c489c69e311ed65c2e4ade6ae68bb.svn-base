...
<table class="report">
    ...
    <?php foreach ($dataProvider->data as $header): ?>
        <tr class="items1">
            <td class="width1-1"><?php echo $header->id . ' - ' . $header->id . ' - ' . CHtml::encode(CHtml::value($header, 'category.name')); ?></td>
            ...
            <td colspan="3">
                <table>
                    <?php foreach ($header->deliveryDetails as $detail): ?>
                        <tr>
                            <td class="width2-1"><?php echo CHtml::encode(CHtml::value($detail, 'deliveryHeader.number')); ?></td>
                            <td class="width2-2"><?php echo CHtml::encode(Yii::app()->dateFormatter->format('d MMMM yyyy', strtotime($detail->deliveryHeader->date))); ?></td>
                            <td class="width2-3"><?php echo CHtml::encode(CHtml::value($detail, 'deliveryHeader.customer.company')); ?></td>														
                    ...
                    <?php endforeach; ?>
                    <tr>
                        <td colspan="3" style="border-top: 1px solid; font-weight: bold; text-align: right ; font-size: small">TOTAL</td>
                        <td style="border-top: 1px solid; font-weight: bold; text-align: right; font-size: small"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', ceil(CHtml::value($header, 'totalQuantitySales')))); ?></td>
                        <td colspan="3" style="border-top: 1px solid; font-weight: bold; text-align: right; font-size: small"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', ceil(CHtml::value($header, 'totalSales')))); ?></td>
                    </tr>
                </table>
            </td>
        </tr>
    ...
    <tr>
        ...
        <td style="border-top: 1px solid; font-weight: bold; text-align: right">TOTAL PENJUALAN</td>
        <td style="border-top: 1px solid; font-weight: bold; text-align: right">
            <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', ceil($this->reportGrandTotal($dataProvider)))); ?>
        </td>
    </tr>
</table>
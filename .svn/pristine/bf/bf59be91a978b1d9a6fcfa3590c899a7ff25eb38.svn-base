...
class DeliveryWarehouseController extends SelectionController {
    ...
    public function filterAccess($filterChain) {
        $taxViewValid = Yii::app()->user->checkAccess('tDeliveryView') || Yii::app()->user->checkAccess('tsDeliveryView');
        $nonTaxViewValid = Yii::app()->user->checkAccess('ntDeliveryView');

        if ($filterChain->action->id === 'memo' || $filterChain->action->id === 'view' || $filterChain->action->id === 'admin') {
            if (!($taxViewValid || $nonTaxViewValid))
...
//		));
    ...
    protected function memoToExcel($delivery) {
...
//		$worksheet->getRowDimension('A3')->setAutoSize(true);
        ...
        $worksheet->setCellValue('A2', 'SURAT JALAN');
        ...
        $worksheet->setCellValue('A4', 'Pengiriman #');
        $worksheet->setCellValue('A5', 'Tanggal');
        $worksheet->setCellValue('A6', 'Info');
        ...
        $worksheet->setCellValue('D4', $delivery->number);
        $worksheet->setCellValue('D5', Yii::app()->dateFormatter->format('d MMMM yyyy', strtotime($delivery->date)));
        $worksheet->setCellValue('D6', $delivery->warehouse->code);
        ...
        $worksheet->setCellValue('G4', 'Customer');
        $worksheet->setCellValue('G5', 'Dibuat oleh');
        $worksheet->setCellValue('G6', 'Pengirim');
        ...
        $worksheet->setCellValue('J4', $delivery->customer->company);
        $worksheet->setCellValue('J5', $delivery->admin->name);
        $worksheet->setCellValue('J6', $delivery->driver);
        ...
        $counter = 9;
        foreach ($delivery->deliveryDetails as $i => $detail) {
            ...
            $worksheet->setCellValue("A{$counter}", CHtml::value($detail, 'product_name'));
        ...
        $worksheet->setCellValue("L{$counter}", CHtml::value($delivery, 'subTotalQuantity'));
        $counter++;
        ...
        $worksheet->setCellValue("C{$counter}", $delivery->note);
        $counter++;
        $counter++;
